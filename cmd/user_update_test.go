...
func TestParseGithubProfile(t *testing.T) {
	tests := []struct {
		...
		{"valid username", "johndoe"},
		{"empty username", ""},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := parseGithubProfile(tt.input)
			if result == nil {
				t.Errorf("parseGithubProfile(%q) returned nil", tt.input)
			}
			if _, ok := result["Github"]; !ok {
				t.Errorf("parseGithubProfile(%q) missing 'Github' key", tt.input)
			}
		})
	}
}

func TestParseAmazonUsername(t *testing.T) {
	tests := []struct {
		name  string
		input string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := parseAmazonUsername(tt.input)
			if result == nil {
				t.Errorf("parseAmazonUsername(%q) returned nil", tt.input)
			}
			if _, ok := result["Amazon"]; !ok {
				t.Errorf("parseAmazonUsername(%q) missing 'Amazon' key", tt.input)
			}
		})
	}
}

func TestParseVpnRole(t *testing.T) {
	tests := []struct {
		name  string
		input string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := parseVpnRole(tt.input)
			if result == nil {
				t.Errorf("parseVpnRole(%q) returned nil", tt.input)
			}
			if _, ok := result["OpenVPN"]; !ok {
				t.Errorf("parseVpnRole(%q) missing 'OpenVPN' key", tt.input)
			}
		})
	}
}
